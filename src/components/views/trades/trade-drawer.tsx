  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* THE BODY IS SHARED WITH `/trades/[id]` (`S5d`, 2026-08-20). Everything from the 64px
            mark down to the broker ids now lives in `TradeDetail`, because the phone gets a route
            and the desktop keeps this drawer — two containers, one body. Copying it would have
            forked the one surface that shows gross, fees and net together, which is the reconcile
            claim itself.
            THE SCROLL CONTAINER STAYS HERE, and that is the split: `TradeDetail` owns the facts and
            no chrome, this owns the panel, the scrollport and the way out. */}
        <div className="min-h-0 flex-1 overflow-y-auto">
          <TradeDetail trade={t} zone={zone} titleId="trade-drawer-title" />
        </div>
      </div>
    </div>
  );
