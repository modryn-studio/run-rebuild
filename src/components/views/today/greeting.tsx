...
export function Greeting({ text }: { text: string }) {
  return (
    <HeaderSlot slot="title">
      {/* `max-md:hidden` (2026-09-03). The phone's band carries the page name instead - centred,
          at `text-h3`, exactly as `/accounts` and `/trades` carry theirs - because that is what the
          REFERENCE'S APP does: its dashboard bar reads `Dashboard`, and the greeting is a thing
          only its web build has. It pairs with `SELF_TITLED`'s `md:hidden` on the shell's `<h1>`,
          so the two can never occupy the band's centre at the same time. */}
      <span className="text-title text-text hidden truncate font-medium md:block">{text}</span>
    </HeaderSlot>
  );
}
