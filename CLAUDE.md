...
- **An overlay owns ONE history entry, however many screens are inside it**, and `useOverlayBack` decides who answers a pop from a module array — never from `history.state`, which Next copies forward onto neighbouring entries. Both rules exist because breaking either made the phone's filter apply "sometimes".
