...
- **A drill-down page on a phone is a `fixed inset-0` panel that slides itself in** (`.sheet-transition`, no `loading.tsx` on that segment or the entrance runs twice). **`<ViewTransition>` is NOT available**: react 19.2.7 stable does not export it and Next 16.3 only vendors the experimental React behind four experimental flags. `design-system.md` §6b.
