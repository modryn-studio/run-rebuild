...
description: >
  Reverse-lookup glossary that turns a vague description of a web animation or motion effect
  into its exact term ("the bouncy thing when a popover opens" -> Pop in; "the iOS rubber-band
  scroll" -> Rubber-banding). For naming an effect, not designing or building one - see
  design-rules.md (modryn-hq) for what this project actually does. Trigger on "what's this
  animation called", "what do you call the effect where...", or describing motion without
  knowing its name.
---

<!-- Sync: vendored, unmodified, from https://github.com/emilkowalski/skills
     (skills/animation-vocabulary/SKILL.md), read 2026-08-19. Descriptive glossary, not a
     rules skill - it names effects, it does not prescribe curves, durations, or when to
     animate, so it does not conflict with anything in design-rules.md. Three of its siblings
     in that repo DO conflict (active:scale(0.97) as a requirement, a different --ease-out)
     and were deliberately left out; see modryn-hq:playbooks/ui-ux-sources.md for the full
     assessment. Re-pull from upstream if this ever drifts; do not hand-edit the glossary
     terms, only the frontmatter above if the trigger phrasing needs to change. -->
